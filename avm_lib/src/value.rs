...
where
    T: num::Float + num::cast::ToPrimitive,
{
    let val;
    match op {
        OpType::Add => val = x + y,
    ...
    fn add(self, other: Type) -> Result<Type, &'static str> {
        match (self, other) {
            ...
            (Type::Short(x), Type::Char(y)) => {
                checked_op_integer(x, y as i16, RetType::Short, OpType::Add)
            }
            (Type::Int(x), Type::Char(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Add)
            }
            (Type::Float(x), Type::Char(y)) => {
                checked_op_float(x, y as f32, RetType::Float, OpType::Add)
            }
            (Type::Double(x), Type::Char(y)) => {
                checked_op_float(x, y as f64, RetType::Double, OpType::Add)
            }
            //Short
            (Type::Char(x), Type::Short(y)) => {
                checked_op_integer(x as i16, y, RetType::Short, OpType::Add)
            }
            (Type::Short(x), Type::Short(y)) => {
                checked_op_integer(x, y, RetType::Short, OpType::Add)
            }
            ...
            (Type::Char(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Add)
            }
            ...
            (Type::Char(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Add)
            }
            ...
            (Type::Char(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Add)
            }
            ...
            (Type::Double(x), Type::Double(y)) => {
                checked_op_float(x, y, RetType::Double, OpType::Add)
            }
        }
    }
}
...
impl Sub for Type {
    ...
    fn sub(self, other: Type) -> Result<Type, &'static str> {
        match (self, other) {
            ...
            (Type::Short(x), Type::Char(y)) => {
                checked_op_integer(x, y as i16, RetType::Short, OpType::Sub)
            }
            (Type::Int(x), Type::Char(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Sub)
            }
            (Type::Float(x), Type::Char(y)) => {
                checked_op_float(x, y as f32, RetType::Float, OpType::Sub)
            }
            (Type::Double(x), Type::Char(y)) => {
                checked_op_float(x, y as f64, RetType::Double, OpType::Sub)
            }
            //Short
            (Type::Char(x), Type::Short(y)) => {
                checked_op_integer(x as i16, y, RetType::Short, OpType::Sub)
            }
            (Type::Short(x), Type::Short(y)) => {
                checked_op_integer(x, y, RetType::Short, OpType::Sub)
            }
            (Type::Int(x), Type::Short(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Sub)
            }
            ...
            //Int
            (Type::Char(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Sub)
            }
            (Type::Short(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Sub)
            }
            ...
            //Float
            (Type::Char(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Sub)
            }
            (Type::Short(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Sub)
            }
            ...
            //Double
            (Type::Char(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Sub)
            }
            (Type::Short(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Sub)
            }
            ...
            (Type::Double(x), Type::Double(y)) => {
                checked_op_float(x, y, RetType::Double, OpType::Sub)
            }
        }
    }
}
...
impl Mul for Type {
    ...
    fn mul(self, other: Type) -> Result<Type, &'static str> {
        match (self, other) {
            ...
            (Type::Short(x), Type::Char(y)) => {
                checked_op_integer(x, y as i16, RetType::Short, OpType::Mul)
            }
            (Type::Int(x), Type::Char(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Mul)
            }
            (Type::Float(x), Type::Char(y)) => {
                checked_op_float(x, y as f32, RetType::Float, OpType::Mul)
            }
            (Type::Double(x), Type::Char(y)) => {
                checked_op_float(x, y as f64, RetType::Double, OpType::Mul)
            }
            //Short
            (Type::Char(x), Type::Short(y)) => {
                checked_op_integer(x as i16, y, RetType::Short, OpType::Mul)
            }
            (Type::Short(x), Type::Short(y)) => {
                checked_op_integer(x, y, RetType::Short, OpType::Mul)
            }
            (Type::Int(x), Type::Short(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Mul)
            }
            ...
            //Int
            (Type::Char(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Mul)
            }
            (Type::Short(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Mul)
            }
            ...
            //Float
            (Type::Char(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Mul)
            }
            (Type::Short(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Mul)
            }
            ...
            //Double
            (Type::Char(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Mul)
            }
            (Type::Short(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Mul)
            }
            ...
            (Type::Double(x), Type::Double(y)) => {
                checked_op_float(x, y, RetType::Double, OpType::Mul)
            }
        }
    }
}
...
impl Div for Type {
    ...
    fn div(self, other: Type) -> Result<Type, &'static str> {
        match (self, other) {
            ...
            (Type::Short(x), Type::Char(y)) => {
                checked_op_integer(x, y as i16, RetType::Short, OpType::Div)
            }
            (Type::Int(x), Type::Char(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Div)
            }
            (Type::Float(x), Type::Char(y)) => {
                checked_op_float(x, y as f32, RetType::Float, OpType::Div)
            }
            (Type::Double(x), Type::Char(y)) => {
                checked_op_float(x, y as f64, RetType::Double, OpType::Div)
            }
            //Short
            (Type::Char(x), Type::Short(y)) => {
                checked_op_integer(x as i16, y, RetType::Short, OpType::Div)
            }
            (Type::Short(x), Type::Short(y)) => {
                checked_op_integer(x, y, RetType::Short, OpType::Div)
            }
            (Type::Int(x), Type::Short(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Div)
            }
            ...
            //Int
            (Type::Char(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Div)
            }
            (Type::Short(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Div)
            }
            ...
            //Float
            (Type::Char(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Div)
            }
            (Type::Short(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Div)
            }
            ...
            //Double
            (Type::Char(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Div)
            }
            (Type::Short(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Div)
            }
            ...
            (Type::Double(x), Type::Double(y)) => {
                checked_op_float(x, y, RetType::Double, OpType::Div)
            }
        }
    }
}
...
impl Rem for Type {
    ...
    fn rem(self, other: Type) -> Result<Type, &'static str> {
        match (self, other) {
            ...
            (Type::Short(x), Type::Char(y)) => {
                checked_op_integer(x, y as i16, RetType::Short, OpType::Rem)
            }
            (Type::Int(x), Type::Char(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Rem)
            }
            (Type::Float(x), Type::Char(y)) => {
                checked_op_float(x, y as f32, RetType::Float, OpType::Rem)
            }
            (Type::Double(x), Type::Char(y)) => {
                checked_op_float(x, y as f64, RetType::Double, OpType::Rem)
            }
            //Short
            (Type::Char(x), Type::Short(y)) => {
                checked_op_integer(x as i16, y, RetType::Short, OpType::Rem)
            }
            (Type::Short(x), Type::Short(y)) => {
                checked_op_integer(x, y, RetType::Short, OpType::Rem)
            }
            (Type::Int(x), Type::Short(y)) => {
                checked_op_integer(x, y as i32, RetType::Int, OpType::Rem)
            }
            ...
            //Int
            (Type::Char(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Rem)
            }
            (Type::Short(x), Type::Int(y)) => {
                checked_op_integer(x as i32, y, RetType::Int, OpType::Rem)
            }
            ...
            //Float
            (Type::Char(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Rem)
            }
            (Type::Short(x), Type::Float(y)) => {
                checked_op_float(x as f32, y, RetType::Float, OpType::Rem)
            }
            ...
            //Double
            (Type::Char(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Rem)
            }
            (Type::Short(x), Type::Double(y)) => {
                checked_op_float(x as f64, y, RetType::Double, OpType::Rem)
            }
            ...
            (Type::Double(x), Type::Double(y)) => {
                checked_op_float(x, y, RetType::Double, OpType::Rem)
            }
        }
    }
}
