...
fn parse_instruction(
    s: &String,
) -> Result<fn(Option<value::Type>, &mut Vec<value::Type>) -> stack::ExecState, &'static str> {
    match s.as_ref() {
...
fn parse_value(s: &String) -> Result<value::Type, &'static str> {
    ...
    if let (1, 1) = (v1.len(), v2.len()) {
        ...
        match (parse_value_type(&split[0]), parse_value_syntax(&split[1])) {
            (ValType::Char, ValFamily::Integer) => match split[1].parse::<i8>() {
                Ok(val) => return Ok(value::Type::Char(val)),
                Err(_) => return Err(ERR_VALUE_SYNTAX),
            },
            (ValType::Short, ValFamily::Integer) => match split[1].parse::<i16>() {
                Ok(val) => return Ok(value::Type::Short(val)),
                Err(_) => return Err(ERR_VALUE_SYNTAX),
            },
            (ValType::Int, ValFamily::Integer) => match split[1].parse::<i32>() {
                Ok(val) => return Ok(value::Type::Int(val)),
                Err(_) => return Err(ERR_VALUE_SYNTAX),
            },
            (ValType::Float, ValFamily::Float) => match split[1].parse::<f32>() {
                Ok(val) => return Ok(value::Type::Float(val)),
                Err(_) => return Err(ERR_VALUE_SYNTAX),
            },
            (ValType::Double, ValFamily::Float) => match split[1].parse::<f64>() {
            ...
            | (ValType::Int, _)
            | (ValType::Float, _)
            | (ValType::Double, _) => return Err(ERR_VALUE_SYNTAX),
...
fn generate_token(line: &str, line_nb: &u64) -> Token {
    ...
    match vec_splited.len() {
        ...
        1 => match parse_instruction(&vec_splited[0]) {
