...
fn avm_lib_add_float_failed() {
	assert_eq!(Err(value::ERR_OVERFLOW), Type::Float(std::f32::MAX) + Type::Float(std::f32::MAX));
}
...
fn avm_lib_add_double_failed() {
	assert_eq!(Err(value::ERR_OVERFLOW), Type::Double(std::f64::MAX) + Type::Double(std::f64::MAX));
}
